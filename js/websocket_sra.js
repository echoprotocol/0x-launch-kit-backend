...
var WebsocketSRA = /** @class */ (function() {
    function WebsocketSRA(server, opts) {
    ...
    WebsocketSRA._decodedContractAndAssetData = function(assetData) {
        var e_1, _a;
        var data = [assetData];
        ...
        if (order_utils_1.assetDataUtils.isMultiAssetData(decodedAssetData)) {
            try {
                for (var _b = __values(decodedAssetData.nestedAssetData), _c = _b.next(); !_c.done; _c = _b.next()) {
                    var nested = _c.value;
                    data = __spread(data, WebsocketSRA._decodedContractAndAssetData(nested).data);
                }
            } catch (e_1_1) {
                e_1 = { error: e_1_1 };
            } finally {
    ...
    WebsocketSRA._matchesOrdersChannelSubscription = function(order, opts) {
        ...
        var makerContractAndAssetData = WebsocketSRA._decodedContractAndAssetData(makerAssetData);
        var takerContractAndAssetData = WebsocketSRA._decodedContractAndAssetData(takerAssetData);
        if (
    ...
    WebsocketSRA.prototype.destroy = function() {
        var e_2, _a;
    ...
    WebsocketSRA.prototype.orderUpdate = function(apiOrders) {
    ...
    WebsocketSRA.prototype._processConnection = function(ws, _req) {
    ...
    WebsocketSRA.prototype._processMessage = function(ws, data) {
        var message;
        try {
    ...
    WebsocketSRA.prototype._cleanupConnections = function() {
        var e_6, _a;
        try {
    ...
    WebsocketSRA.prototype._messageHandler = function(ws) {
        var _this = this;
    ...
    WebsocketSRA.prototype._processError = function(ws, err) {
        var errorBody = error_handling_1.generateError(err).errorBody;
        ws.send(JSON.stringify(errorBody));
        ws.terminate();
    };
    // tslint:disable-next-line:prefer-function-over-method
    WebsocketSRA.prototype._pongHandler = function(ws) {
        return function() {
            ws.isAlive = true;
        };
    };
    WebsocketSRA.prototype._closeHandler = function(ws) {
        var _this = this;
        return function() {
            var e_7, _a;
            try {
